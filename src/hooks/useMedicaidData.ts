...
export function useMedicaidData() {
    ...
    useEffect(() => {
        const columns: MedicaidColumn[] = medicaidData.medicaidTableJson.columnNames.map((rawColumn) => {
            const column: MedicaidColumn = {
                ...
                ageMinimum: 19,
                ageMaximum: null,
                isForPregnantWomen: rawColumn.parsedString.toLowerCase().includes("pregnant"),
                isFamily: false,
            };
            ...
            if (column.parsedString.toLowerCase().includes("ages")) {
                ...
                const strAgeRange = splitColumnName.filter((strSplitName) => strSplitName.indexOf("-") >= 0);
                
                if (strAgeRange.length != 1) throw "Invalid age range: multiple elements containing -";
                const splitAgeRange = strAgeRange[0].split("-");
                if (splitAgeRange.length != 2) throw "Invalid age range: does not contain a min and max";
            ...
            // Check for isFamily
            column.isFamily = (column.parsedString.toLowerCase().includes("parent") || column.parsedString.toLowerCase().includes("caregiver"));
    ...
    useEffect(() => {
        ...
        const rows: MedicaidRow[] = medicaidData.medicaidTableJson.data.map((rawRow) => {
            return {
    ...
    const getColumns = (
        ...
        if (age)
          filters.push((column) => column.isState || (column.ageMinimum <= age && (column.ageMaximum == null || column.ageMaximum >= age)));
        ...
        if (familySize && familySize > 1)
            ...
            filters.push((column) => column.isState || column.isForPregnantWomen == isPregnant);
        ...
        if (filteredColumns && columns.length != filteredColumns.length) {
            filteredRows = filteredRows.map((filteredRow) => {
                return {
                    ...
                    data: filteredRow.data.filter((cell) => filteredColumns.findIndex((filteredColumn) => filteredColumn.parsedString == cell.column.parsedString) >= 0)
                }
            });
        }
